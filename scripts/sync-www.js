...
const files = ["index.html","styles.css","app.js","firebase-config.js","manifest.webmanifest","sw.js"];
...
for (const icon of fs.readdirSync(path.join(root, "icons"))) {
  if (icon.endsWith(".png")) fs.copyFileSync(path.join(root, "icons", icon), path.join(www, "icons", icon));
}
